...
void main() {
  ...
  late DriftRelatorioObraRepository repository;
  ...
  setUp(() async {
    ...
    repository = DriftRelatorioObraRepository(database);
  ...
  group('Story 7.1 - RelatorioObraRepository', () {
    test('carrega dados completos da obra para relatorio', () async {
    ...
    test('rejeita obra inexistente', () async {
      expect(
        () => repository.carregarDadosDaObra('obra-inexistente'),
        throwsA(isA<ObraRelatorioNaoEncontradaException>()),
      );
    });
  });
}
...
Future<void> _popularDadosBase(db.AppDatabase database) async {
  ...
  await database.into(database.obras).insert(
        db.ObrasCompanion.insert(
          ...
          progressoFisico: const Value(60),
          progressoPrazoDias: const Value(5),
        ),
      );
